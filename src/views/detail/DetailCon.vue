<template>
  <section :class="['detail', {dark:darkMode}, work.sub.path]">
    ...
    <section class="detail-intro">
      <div class="obj"></div>
      <div class="txt">
        <h1 data-aos="fade-right">{{work.title}}</h1>
        <p data-aos="fade-right" data-aos-delay="150">{{work.do}}</p>
      </div>
    </section>
    ...
    <div class="floating-btns is-show scr-up" v-if="work.code">
      <button type="button" class="btn-ico btn-black" aria-label="Code 보기" @click="newWindow(work.code)">View Code</button>
    </div>
  </section>
</template>
<script>
